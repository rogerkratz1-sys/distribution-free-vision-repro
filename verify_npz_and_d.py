...
for name,(a_k,b_k) in (("davg",("d_null","d_cluster")),("mutual",("m_null","m_cluster"))):
    a,b = (lambda x: x[1:].copy() if x.size>0 and x[0] in (0.0,49.5) else x)(npz[a_k].astype(float)), (lambda x: x[1:].copy() if x.size>0 and x[0] in (0.0,49.5) else x)(npz[b_k].astype(float))
